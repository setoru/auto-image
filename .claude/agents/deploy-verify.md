...
- **新格式以可执行断言判定**：含 `# 验证类型: required` 的代码块作为一个整体执行；仅当 `exit_code = 0` 且 stdout 含 `VERIFY_PASS:` 时通过。`# 期望:` 只用于报告展示，不解析其中的自然语言逻辑。
- **诊断项不阻断流水线**：含 `# 验证类型: diagnostic` 的代码块全量执行并记录，但不计入通过率，也不影响整体结论。
- **兼容旧格式指南**：没有 `# 验证类型:` 的代码块沿用旧逻辑，逐条执行命令并按期望关键字/子串匹配；不得把旧格式静默当成新格式断言。
...
按文档顺序扫描验证指南中的所有 ```` ```bash ```` 代码块，提取 `# 验证类型:`、`# 期望:` 及其最近的上级标题作为检查项名称。不得依赖固定章节编号或标题；systemd、SysV、Docker Compose 和纯 CLI 软件可以有不同的验证结构。

解析与兼容规则：

- 新格式：含 `# 验证类型: required|diagnostic`；整个代码块是一条检查项，不拆分其中的赋值、条件分支或协议回退命令。
- 旧格式：不含 `# 验证类型:`；沿用原行为，从代码块提取命令和对应的 `# 期望:` 后逐条检查。
- 每项记录：章节、验证类型、完整代码块或命令、期望输出。含变更类命令（安装/修改/启停服务/写文件）的项**跳过**并标注「跳过（非验证类）」。
- 若指南未解析出任何 `required` 或旧格式检查项，视为验证指南无效，整体结论记为 ❌ 失败；不得以 0/0 判定为通过。
...
```
- 调用 `ssh_execute.py` 时，必须将完整代码块作为一个经过 shell 安全引用的字面量参数传入，确保其中的引号、换行、`$变量` 和 `$()` 不在本地 shell 提前展开；远端收到的命令内容必须与指南代码块一致。
