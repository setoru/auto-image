...
- **全量执行、不失败即停**：与安装不同，验证需**逐项全部执行**并记录每项结果，最后汇总；单项失败不中断后续检查，以给出完整画像。唯一例外是下方「永久性连接故障」——SSH 链路本身不可用时继续执行没有意义。
...
- **基础设施异常不等同于软件失败**：`exit_code=-1` 表示远端命令根本没跑完（本地超时/连接中断/脚本自身异常），**一律不得判为软件失败**；`exit_code=255` 且 stdout 为空同理。这类结果对 `required`/`legacy` 项先原样重试一次，仍不可判定则记为 `⚠️ 无法判定`，不计入通过或失败，**不得下结论说软件未安装**。远端命令自身有输出并返回 255 时按业务失败处理；`diagnostic` 项只记录异常，不重试、不影响整体结论。判别字符串与归档细则见步骤 4。
- **永久性连接故障停止整轮**：认证失败、host key 校验失败、别名/配置错误、域名解析失败属永久故障，重试无意义。命中时**停止后续检查项**，整体结论记为 `⚠️ 无法判定`、原因写「基础设施异常」，并说明需修复 SSH 配置后重跑——软件是否装好同样未能判定，不得记为 ❌ 软件失败。
- **命令超时不自动归为基础设施异常**：远端命令超时既可能是链路问题，也可能是服务卡死（真实软件故障）。重试后仍超时时，用「本轮**其他检查项**是否有成功返回」区分：有则记 ❌ 软件失败（服务无响应），全都超时或异常才记 `⚠️ 无法判定`。步骤 2 的连通性探测不算在内，该判断在全部检查项执行完毕后统一回填。
...
- 同一位置若出现 `> 远程前提: sudo-nopasswd`（同样只接受半角冒号），记录该前提并写入结果文件的「整体结论」段；执行时不额外做任何事，仅在出现「前提未满足」类异常时用于归因。未声明该标记却出现 sudo 提密，说明指南违反了 deploy-guide 的约定，须在建议中指出。
...
- 异常分类（按顺序判别，命中即停止后续分类）：
  ...
  - **传输异常**：`exit_code=255`、stdout 为空，且 stderr 含 `Connection timed out`、`Operation timed out`、`Connection refused`、`Connection reset`、`Connection closed`、`Broken pipe`、`No route to host`、`Network is unreachable`、`kex_exchange_identification`、`channel open failed`、`SSHException` 之一。
  - **本地执行异常（兜底）**：`exit_code=-1` 未命中上述任一类，或 `exit_code=255` 且 stdout 为空但 stderr 未命中传输异常关键字。ssh-skill 有 daemon / native / paramiko 三条执行路径，各自的错误文案不同（如 daemon 的 `命令执行错误:`、`SSH 连接已断开且重连失败`、`守护进程处理错误:`，paramiko 的 `Execution error:`），关键字清单不可能穷尽——**凡 `exit_code=-1` 一律落入本类，绝不判为软件失败**。
- 重试：仅对 `required` 和 `legacy` 项、且仅对上述**命令超时 / 前提未满足 / 传输异常 / 本地执行异常**四类（即除「永久性连接故障」外的全部异常），使用完全相同的命令和超时立即重试一次。第二次成功则按第二次结果判定；第二次返回普通业务失败则记为 ❌。`diagnostic` 项不重试。
- 重试后仍异常的归档：
  - 传输异常 / 本地执行异常 → `⚠️ 无法判定`，记录两次 stderr。
  - 命令超时 → 若本轮**其他检查项**有成功返回（链路正常），记 ❌ 软件失败，差异写「命令超时，服务无响应或超时值过短」；若本轮所有检查项均超时或异常，记 `⚠️ 无法判定`。步骤 2 的连通性探测不计入；该判断在全部检查项执行完毕后统一回填，不依赖执行顺序。
  - 前提未满足 → `⚠️ 无法判定`，建议写明：验证指南应在文档头声明 `> 远程前提: sudo-nopasswd` 并将 sudo 命令改为 `sudo -n`，或改用无需特权的只读命令。
- `required`：先排除并处理上述异常；其余结果仅当 `exit_code = 0` 且 stdout 含 `VERIFY_PASS:` 时记为 ✅，否则记为 ❌。不得再用 `# 期望:` 文本覆盖该结果。
