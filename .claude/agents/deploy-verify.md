...
- **不可判定不等同于软件失败**：`exit_code=-1` 表示远端命令根本没跑完（本地超时/连接中断/脚本自身异常），**一律不得判为软件失败**；`exit_code=255` 且 stdout 为空同理。这类结果对 `required`/`legacy` 项先原样重试一次，仍不可判定则记为 `⚠️ 无法判定`，不计入通过或失败，**不得下结论说软件未安装**。远端命令自身有输出并返回 255 时按业务失败处理；`diagnostic` 项只记录异常，不重试、不影响整体结论。判别与归档细则见步骤 4。
- **超时同样是不可判定**：远端命令超时既可能是链路问题，也可能是服务卡死或超时值过短，仅凭超时无法区分。重试后仍超时一律记 `⚠️ 无法判定`、原因「命令超时」，把实际耗时与 stderr 摆进报告，不替读者下「软件坏了」的结论。
...
- 不可判定分类（按顺序判别，命中即停止后续分类）：
  ...
  - **连接或执行异常（兜底）**：`exit_code=-1` 未命中上述两类，或 `exit_code=255` 且 stdout 为空。ssh-skill 有 daemon / native / paramiko 三条执行路径，错误文案各不相同（daemon 的 `命令执行错误:`、`SSH 连接已断开且重连失败`，native 的 `Connection refused`、`Permission denied`，paramiko 的 `Execution error:`），关键字清单不可能穷尽，也无需穷尽——**凡 `exit_code=-1`、或 `255` 且 stdout 为空，一律落入本类，绝不判为软件失败**，原始 stderr 原样写进报告供排障。
- 重试：仅对 `required` 和 `legacy` 项、且仅对上述三类不可判定情形，用完全相同的命令和超时立即重试一次。已判 ✅ 或普通业务失败 ❌ 的项**不重试**。第二次成功则按第二次结果判定；第二次返回普通业务失败则记为 ❌；第二次仍命中上述任一类则记为 `⚠️ 无法判定`，并记录两次 stderr。`diagnostic` 项不重试。
- 「前提未满足」的建议须写明：验证指南应在文档头声明 `> 远程前提: sudo-nopasswd` 并将 sudo 命令改为 `sudo -n`，或改用无需特权的只读命令。
...
- **单项失败不中断**，继续执行其余检查项。
- 汇总：只汇总必选检查项与旧格式检查项的通过数/总数；诊断项单独统计。存在至少一个软件失败项时整体结论为 ❌；没有软件失败但存在无法判定项时为 `⚠️ 无法判定`；所有必选/旧格式检查项均通过且无无法判定项时才为 ✅。每个无法判定项必须逐项写明原因（取值见下方结果文件模板的「无法判定原因」），不得写成软件未安装。（下游是否继续由编排层按整体结论决定，本 agent 不调用 archive。）
