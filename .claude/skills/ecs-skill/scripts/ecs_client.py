#!/usr/bin/env python
"""ecs_client —— 华为云 ECS 客户端构造 + scope/凭证解析。
...
ECS_ENDPOINT_TEMPLATE = "ecs.{region}.myhuaweicloud.com"
SCRIPT_DIR = Path(__file__).resolve().parent
# 仓库根 = .claude/skills/ecs-skill/scripts/ 起 4 级上 —— 共享 scope.yaml 所在。
...
def build_client(creds: Credentials) -> EcsClient:
    """官方 EcsClient（取代原 CustomClient 手搓 HTTP）。"""
    endpoint = f"https://{ECS_ENDPOINT_TEMPLATE.replace('{region}', creds.region)}"
